...
impl WeightedInnerProdArg {
    pub fn prove(
        ...
        if n != 1 {
            ...
            let yn_aR = (0..n)
                .map(|i| {
                    BigInt::mod_mul(&powers_y[n-1], &a_R[i], &order)
                })
                .collect::<Vec<BigInt>>();
            let yninv_aL = (0..n)
                .map(|i| {
                    BigInt::mod_mul(&powers_yinv[n-1], &a_L[i], &order)
                })
                .collect::<Vec<BigInt>>();
            ...
            L_vec.push(L);
            R_vec.push(R);
            return WeightedInnerProdArg::prove(&G_new, &H_new, &g, &h, &P, &a_new, &b_new, &alpha_new, L_vec, R_vec);
        }
        else {
    ...
    pub fn verify(&self, g_vec: &[GE], hi_tag: &[GE], g: &GE, h: &GE, P: &GE) -> Result<(), Errors> {
        let G = &g_vec[..];
        let H = &hi_tag[..];
        let n = G.len();
        let order = FE::q();
    ...
    /// 
    pub fn fast_verify(&self, g_vec: &[GE], hi_tag: &[GE], g: &GE, h: &GE, P: &GE)
    -> Result<(), Errors> {
        let G = &g_vec[..];
        let H = &hi_tag[..];
        let n = G.len();
        ...
        for (Li, Ri) in self.L.iter().zip(self.R.iter()) {
            ...
            minus_e_sq_x_inv_sq_vec.push(BigInt::mod_sub(&BigInt::zero(), &e_sq_x_inv_sq_bn, &order));
...
mod tests {
    ...
    use proofs::weighted_inner_product::{weighted_inner_product, scalar_powers};
    ...
    fn test_wip(){
        ...
        let b: Vec<BigInt> = vec![BigInt::from(1), BigInt::from(2), BigInt::from(3), BigInt::from(4)];
        let y = BigInt::from(2);

        let y_powers = scalar_powers(y.clone(), 4);
        let expect_y_powers = vec![BigInt::from(2), BigInt::from(4), BigInt::from(8), BigInt::from(16)];
        assert_eq!(y_powers, expect_y_powers, "Scalar powers of y fails!");

        let a_weighted_b = weighted_inner_product(&a, &b, y.clone());
        assert_eq!(a_weighted_b, BigInt::from(46), "Weighted inner product fails!");
    }

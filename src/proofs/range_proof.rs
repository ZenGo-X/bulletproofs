...
impl RangeProof{
    ...
    pub fn prove(g_vec: &Vec<GE>, h_vec: &Vec<GE>, G: &GE, H: &GE, secret: &FE, blinding: &FE, bit_length: usize) -> RangeProof{
        ...
        let secret_bits = (0..bit_length).map(|i|{
    ...
    pub fn verify(&self,g_vec: &Vec<GE>, h_vec: &Vec<GE>,G:&GE,  H: &GE, ped_com: &GE ,bit_length: usize) -> Result<bool,Errors> {
        ...
        let scalar_mul_yn = yi.iter().fold(BigInt::zero(),|acc,x| BigInt::mod_add(&acc, x, &order));

        let two = BigInt::from(2);
        let vec_2n = (0..bit_length).map(|i|{
            BigInt::mod_pow(&two, &BigInt::from(i as u32), &order)
        }).collect::<Vec<BigInt>>();

        let scalar_mul_2n = vec_2n.iter().fold(BigInt::zero(),|acc,x: &BigInt| BigInt::mod_add(&acc, x, &order));
        ...
        let challenge_x = HSha256::create_hash(&[&self.tau_x.to_big_int(),&self.miu.to_big_int(), &self.tx.to_big_int()]);
        let challenge_x: FE = ECScalar::from(&challenge_x);
        let Gx = G.clone() * &challenge_x;
        // P' = u^{xc}
        ...
        let P = (0..bit_length).map(|i|{
            g_vec[i].clone() * &z_minus_fe
        }).fold(P1, |acc, x| acc + x);
